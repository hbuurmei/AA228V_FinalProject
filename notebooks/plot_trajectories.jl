...
# ╔═╡ ba117990-f00c-11ef-1e7a-e5f325e03039
begin
	cd("/home/romeo/Documents/Stanford/AA228V_FinalProject/")
	ENV["JULIA_CONDAPKG_BACKEND"] = "Null"
    ENV["JULIA_PYTHONCALL_EXE"] = "/home/romeo/Documents/Stanford/AA228V_FinalProject/.venv/bin/python"
	using AA228V_FinalProject
	using AA228V_FinalProject.PythonCall
	using AA228V_FinalProject: AdditiveNoiseSensor, System, rollout, RLAgent, expert_agent, imitation_agent, Ps, get_depth, Da, Ds, Do, LTLSpecification
	using AA228V_FinalProject.Distributions
	using Plots
	using AA228V_FinalProject.IntervalSets
	using AA228V_FinalProject.SignalTemporalLogic
	using LaTeXStrings
end

# ╔═╡ 8c243519-0dbe-4a1c-b2e7-2de68314cb1e
sensor = AdditiveNoiseSensor(Product([Normal(0, 0.1),
                                 Normal(0.0, 0.1),
                                 Normal(0, 0.15),
                                 Normal(0, 0.15)]))

# ╔═╡ 81decf82-215a-4957-9972-9171b0232d92
sys = System(
    RLAgent(imitation_agent),
    CartPole(; render=false),
    sensor
)

# ╔═╡ bfd7f62d-5855-4c35-b4e5-8f79f328d4c6
qτ = NominalTrajectoryDistribution(Ps(sys.env),
  DisturbanceDistribution(
    (o) -> Da(sys.agent, o),
    (s, a) -> Ds(sys.env, s, a),
    (s) -> Product([Normal(0, 0.1),
                    Normal(0.0, 0.1),
                    Normal(0, 1.15*0.15),
                    Normal(0, 1.15*0.15)])),
                    get_depth(sys)
...
# ╔═╡ 2784975b-cfc2-454d-acda-6ce25d78b698
τs = [rollout(sys, qτ) for _ in 1:100]

# ╔═╡ bea1e579-7bf9-47ff-bc92-3bfa28b116e0
ψ = LTLSpecification(@formula □(s->s[1] ∈ 0±2.4) ∧ □(s->rad2deg(s[3]) ∈ 0±12))

# ╔═╡ f83fd9eb-2d61-48bd-81de-03762b59baf8
function plot_trajectories(τs)
	p = plot(eachindex(τs[1]), x->0; ribbon=deg2rad(12), c="green", fillalpha=0.2, label="", yformatter=y->string(round(Int, rad2deg(y)))*"°", yticks=deg2rad.([-18, -12, -6, 0, 6, 12, 18]), tickfont=12, guidefont=14, size=(500, 300), framestyle=:box)
    for τ in τs
        idx = findfirst(step->(step.s[1]∉(0±2.4)), τ)
        τ = (isnothing(idx) ? τ : τ[1:idx])
        plot!(p, [step.s[3] for step in τ]; color=(isfailure(ψ, τ) ? "red" : "green"), label=""); !isnothing(idx) && @show (idx, τ[idx].s[[1,3]])
    end
    ylims!(p, (-deg2rad(22), deg2rad(22)))
    xlabel!(p, "time step")
    ylabel!(p, L"\theta")
    p
end

# ╔═╡ 97ed778a-94ce-434f-8896-a4d1f42c7c81
plot_trajectories(τs)

# ╔═╡ ef3dd8de-d44e-4da3-90f7-31417ec5b9bb
savefig("/tmp/trajectories.svg")

# ╔═╡ 00000000-0000-0000-0000-000000000001
PLUTO_PROJECT_TOML_CONTENTS = """
[deps]
AA228V_FinalProject = "2b83b5e6-75ac-4935-a092-400fa6ce651e"
LaTeXStrings = "b964fa9f-0449-5b57-a5c2-d3ea65f4040f"
Plots = "91a5bcdd-55d7-5caf-9e0b-520d859cae80"

[compat]
AA228V_FinalProject = "~0.1.3"
LaTeXStrings = "~1.4.0"
Plots = "~1.40.9"
"""
...
manifest_format = "2.0"
project_hash = "1f18fe73d6938541aa37f3eac3cda4bd410149b7"

[[deps.AA228V_FinalProject]]
deps = ["ComponentArrays", "Distances", "Distributions", "ForwardDiff", "IntervalSets", "LazySets", "LinearAlgebra", "LogExpFunctions", "NearestNeighbors", "Optim", "Optimization", "OptimizationOptimJL", "Polyhedra", "PythonCall", "Random", "SignalTemporalLogic", "StaticArrays", "StatsBase", "YAML"]
git-tree-sha1 = "56f30ebf9974a335aafc3bd6a6bc73bd758d2f7e"
uuid = "2b83b5e6-75ac-4935-a092-400fa6ce651e"
version = "0.1.3"
