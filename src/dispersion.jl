...
function softmin_objective_knn(ξ′::SVector{N,<:Real}, knn_points::Vector{<:SVector{N,<:Real}},
                               tau::Float64) where {N}
...
function optimize_point_knn(points::Vector{SVector{N,Float64}}, p::SVector{N, Float64};
                            k_neighbors::Int=6, tau::Float64=1.0, n_steps::Int=3,
                            tree=KDTree(hcat(points...); leafsize=10), verbose=false) where {N}
...
function main_knn_random_start()
    ...
    println("Minimum distance through brute-force: $(min_dist_brute_force)")
end
...
function maximum_dispersion(pts::AbstractVector{<:AbstractVector}, n_rep=30)
    ...
    candidates = [begin
        ...
        while sample ∉ P; sample = first.(bounds) .+ rand(N) .* (last.(bounds) .- first.(bounds)); end
